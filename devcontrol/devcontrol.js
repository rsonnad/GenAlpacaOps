...
async function loadContext() {
  ...
  const CONTEXT_FILES = [
    ...
    { name: 'sponic-tablet-setup.md', path: 'devdocs/sponic-tablet-setup.md', category: 'docs', gh: 'spaces/admin/devcontrol/devdocs/sponic-tablet-setup.md',
      desc: 'Samsung Galaxy Tab A9+ setup runbook for the SponichTab / LangBang dev tablet. Covers Tailscale Always-on VPN, Wireless debugging, fixed adb tcpip 5555, secure-settings grant, and reboot recovery testing.' },
    ...
    { name: 'SONOSAUTOMATION.md', path: 'devdocs/SONOSAUTOMATION.md', category: 'docs', gh: 'spaces/admin/devcontrol/devdocs/SONOSAUTOMATION.md',
      desc: 'Sonos multi-zone audio reference for Alpaca Playhouse. Documents the 14-zone speaker inventory and DHCP reservations, the UDM Pro network settings required for reliable Sonos S1 operation (kernel vs controller IGMP snooping, BSS transition, multicast enhancement, min data rate, STP), the node-sonos-http-api server on Alpuca (port 5005), troubleshooting history (channel-11 conflict, IGMP drift, retry-rate analysis), and the daily auto-snapshot cron that captures UniFi config to public.network_config_snapshots for drift bisection. Loaded when debugging Sonos dropouts, modifying UniFi settings that touch audio, or restoring a known-working network baseline.' },
