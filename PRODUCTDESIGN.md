...
### 3.5 FlashForge: TCP G-code Over HTTP API
...
### 2026-07-06: Combined Sign + ID on One Page, and Single-Email Prepaid Guests

**Decision:** The signing page (`rentals/signing/index.html`) now accepts an optional `&idt=<upload token>` on the signing link. When present, the signer draws their signature **and** uploads a photo ID on the same page, submitting once — the ID posts to the same `verify-identity` function `rentals/verify.html` uses. `nativeSigningService.sendForSignature` mints that ID token and appends it to the signing URL. For **prepaid / no-deposit guests** (`require_deposit = false`, e.g. Airbnb-booked short-term stays), the send flow skips the extra "lease sent" notification and deposit-request emails and omits all payment content from the signing email — so the guest receives exactly **one** email and completes everything in **one step**.

**Why:** A corporate-booked short-term guest (employer already paid via Airbnb) should not receive deposit/payment emails or be bounced between a signing page and a separate ID-upload page. Threading the ID token through the existing signing link reuses the whole native-signing + identity-verification stack with no new edge function, and is fully backward compatible: links without `&idt=` behave exactly as before, and deposit-required rentals are unchanged. See the short-term guest occupancy agreement (`lease_templates` row "Short-Term Guest Agreement (Byron)") for the no-tenancy / holdover / Booking-Party terms used for these stays.

### 2026-09-01: A Dwelling Assignment Is What Grants a Resident Login

**Decision:** Holding a dwelling assignment in an entitled status (`active`, `pending_contract`, `contract_sent`) is what makes someone a resident of the app, and the database now guarantees the login that follows from it. A trigger on `assignments` (and one on `people.email`, for people whose address arrives later) keeps a pending `user_invitations` row in place for anyone entitled, roled `resident`, expiring one day after the assignment ends rather than the 7-day default — open-ended stays get a year. Invitation expiry is now honored by `shared/auth.js`, which previously ignored it. Creating an invitation sends nothing: `user_invitations` has no triggers or webhooks, and invitation email is always an explicit action from `admin/users.js`.

**Why:** Nothing creates `app_users` rows server-side — there is no trigger on `auth.users`, so the row is created client-side by `shared/auth.js` on first sign-in, and the `app_users` RLS INSERT policy only permits that while a pending invitation exists for the signup email. A person could therefore be on the resident list, have an active assignment and an onboarding email in hand, and still be unable to create any account at all — which is what happened to Luka Cassiano. Making the invitation a consequence of the assignment removes the manual step that was being forgotten. The entitled-status set is deliberately identical to `get_my_space_codes()`, so a login exists exactly when there would be codes to show. The generous expiry exists because the two rules interact: honoring expiry is necessary (an invitation carries a role grant up to admin, and an expired one should not keep granting it indefinitely) but a 7-day window is wrong for a residency, where someone may not open the app for a month.

**Repair path:** `shared/auth.js` now links `person_id` even on the public-fallback path, so an account that lands with the wrong role is still identifiable and an admin fixes it by changing one field instead of hand-writing a database row.

### 2026-09-01: Residents See Only Their Own Space's Door Code
